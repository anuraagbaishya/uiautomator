...
class TestMisc(unittest.TestCase):
    ...
    def test_load(self):
        ...
        self.assertIsNotNone(uiautomator.utils.rect)
        self.assertIsNotNone(uiautomator.utils.point)
    ...
    def test_rect(self):
        ...
        for i in range(10):
            ...
            self.assertEqual(uiautomator.utils.rect(top, left, bottom, right), {"top": top, "left": left, "bottom": bottom, "right": right})

    def test_point(self):
        import random
        for i in range(10):
            x, y = random.randint(0, 1024), random.randint(0, 720)
            self.assertEqual(uiautomator.utils.point(x, y), {"x": x, "y": y})
    ...
    def test_next_port(self):
        with patch('socket.socket') as socket:
            ...
            self.assertEqual(uiautomator.utils.next_local_port(), 9010)

        with patch('socket.socket') as socket:
            socket.return_value.connect_ex.return_value = 1
            uiautomator._init_local_port = 32764
            self.assertEqual(uiautomator.utils.next_local_port(), 9008)
